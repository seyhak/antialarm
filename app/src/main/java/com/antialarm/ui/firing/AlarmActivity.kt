...
class AlarmActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val alarmHour = intent.getIntExtra(AlarmReceiver.EXTRA_ALARM_HOUR, 0)
        val alarmMinute = intent.getIntExtra(AlarmReceiver.EXTRA_ALARM_MINUTE, 0)
        ...
        setContent {
            AntiAlarmTheme {
                AlarmFiringScreen(
                        ...
                        onSnooze = {
                            startService(
                                    AlarmService.createSnoozeIntent(this, alarmId, snoozeMinutes)
                            )
                            finish()
                        }
                )
